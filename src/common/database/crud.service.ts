...
export abstract class CrudService<D extends Delegate, T extends CrudMapType> {
  ...
  public async archive(id: string, user: User) {
    return this.delegate.update({
      where: { id },
      data: { status: false, updatedBy: user.id },
    });
  }

  public async restoreArchived(id: string, user: User) {
    return this.delegate.update({
      where: { id },
      data: { status: true, updatedBy: user.id },
    });
  }
